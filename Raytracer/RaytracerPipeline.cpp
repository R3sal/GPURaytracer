...
namespace RT::GraphicsAPI
{
	...
	bool CameraRayGen::Initialize(GPUScheduler* rtScheduler, DescriptorHeap* rtUAVDescriptorTable, CameraInfo rtCameraData)
	{
		...
		if (!(m_rtCameraRayGenState->SetCS("CS_CameraRayGeneration.cso"))) return false;
	...
	bool SortPrimitives::Initialize(GPUScheduler* rtScheduler, uint32_t iNumPrimitives, AABB rtSceneAABB)
	{
		...
		if (!(m_rtGenMortonCodeState->SetCS("CS_GenerateMortonCodes.cso"))) return false;
		...
		if (!(m_rtSortCountState->SetCS("CS_SortCount.cso"))) return false;
		...
		if (!(m_rtSortPrefixSumState->SetCS("CS_SortPrefixSum.cso"))) return false;
		...
		if (!(m_rtSortSortState->SetCS("CS_SortSort.cso"))) return false;
	...
	bool BuildBVH::Initialize(GPUScheduler* rtScheduler, uint32_t iNumPrimitives)
	{
		...
		if (!(m_rtBuildLeavesState->SetCS("CS_BVHBuildLeaves.cso"))) return false;
		...
		if (!(m_rtBuildState->SetCS("CS_BVHBuild.cso"))) return false;
	...
	bool TraceRays::Initialize(GPUScheduler* rtScheduler, DescriptorHeap* rtUAVDescriptorTable, MeshInfo rtMeshData)
	{
		...
		if (!(m_rtTraceRaysState->SetCS("CS_TraceRays.cso"))) return false;
	...
	bool GenerateFinalImage::Initialize(GPUScheduler* rtScheduler, DescriptorHeap* rtUAVDescriptorTable, DXGI_FORMAT dxTargetFormat)
	{
		...
		if (!(m_rtGenerateImageState->SetCS("CS_GenerateFinalImage.cso"))) return false;
